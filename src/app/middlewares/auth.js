...
const { getUserByEmail } = require('../../db/repo/users.repo');

function sign(user){
  return jwt.sign({ uid:user.id, role:user.role, client_id:user.client_id||null }, process.env.JWT_SECRET, { expiresIn:'15d' });
}
