// /src/app/services/llm.service.js
const fetch = require('node-fetch');
const { getClientConfig } = require('../../db/repo/clients.repo');
...
function buildMessages(prompt, userText, meta={}){
  const sys = prompt?.trim() || `أنت مساعد دعم فني محترف. التزم بمجال العمل فقط. أجب بالعربية.`;
  const ctx = [];
...
async function chatWithOpenRouter({ model, apiKey, messages, temperature=0.4 }){
  const key = apiKey || FALLBACK_OR_KEY; if(!key) throw new Error('missing_openrouter_key');
...
async function generateReply(clientId, text, meta={}){
  ...
  const messages = buildMessages(cfg.business_prompt, text, meta);
  const reply = await chatWithOpenRouter({ model, apiKey, messages });
  return reply || 'تم ✅';
}

module.exports = { generateReply, buildMessages };
