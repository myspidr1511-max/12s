...
const { sign, requireAuth, requireRole, verifyPassword } = require('../middlewares/auth');
const { createAdminIfMissing, updateUserById } = require('../../db/repo/users.repo');
...
router.post('/api/auth/login', async (req,res)=>{
  try{
    ...
    const token = sign(u);
    return res.json({ ok:true, token, user:{ id:u.id, role:u.role, client_id:u.client_id||null, email:u.email } });
  }catch(e){ console.error(e); return res.status(500).json({ok:false}); }
});
